...
// sharedRuntime (pool, watch feed, TTL scanner) across every Create the
// apiserver's storagebackend/factory dispatch makes — CR storage,
...
// NewFactoryBackend applies the schema, dials the pool, starts the watch
// feed and the TTL scanner, and returns a Backend wrapping them.
func NewFactoryBackend(ctx context.Context, cfg Config) (*FactoryBackend, error) {
	...
	sr := &sharedRuntime{pool: pool, feed: feed}
...
// Create returns a Postgres-backed storage.Interface for one resource. The
// returned DestroyFunc is a no-op — the shared runtime outlives any single
// resource and is disposed by Close.
func (b *FactoryBackend) Create(
	...
	s.SetFeed(b.shared.feed)
	return s, func() {}, nil
}

// CreateHealthCheck / CreateReadyCheck use the pool's Ping, a single-conn
// RTT against the server.
...
func (b *FactoryBackend) ping() error {
	if b.shared == nil || b.shared.pool == nil {
		return fmt.Errorf("postgres: backend closed")
	}
...
// Pool exposes the shared connection pool for callers that need to run their
// own SQL (e.g. tests, ad-hoc probes).

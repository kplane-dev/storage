...
// Options is the PostgreSQL backend's flag block. Implements
// registry.Backend so an apiserver aggregator can register it via
// NewOptions() and select it at runtime with --storage-backend=postgres.
type Options struct {
	cfg Config
...
func (o *Options) Name() string { return "postgres" }

// AddFlags binds the --postgres-* flags. Called for every registered backend
// at startup so --help lists every backend's flags.
func (o *Options) AddFlags(fs *pflag.FlagSet) {
	fs.StringVar(&o.cfg.DSN, "postgres-dsn", o.cfg.DSN,
		"PostgreSQL connection string or URL (e.g. postgres://user@host:5432/db?sslmode=disable).")
	fs.StringVar(&o.cfg.Database, "postgres-database", o.cfg.Database,
		"Optional database name override; when set, applied after connection instead of the DSN's database.")
	fs.Int32Var(&o.cfg.MaxConns, "postgres-max-conns", o.cfg.MaxConns,
		"Maximum pool size. Zero picks 4 * GOMAXPROCS.")
	fs.DurationVar(&o.cfg.MaxConnLifetime, "postgres-max-conn-lifetime", o.cfg.MaxConnLifetime,
		"Bounds how long any pool connection stays open. Finite values are required for rolling restarts.")
	fs.DurationVar(&o.cfg.MaxConnIdleTime, "postgres-max-conn-idle-time", o.cfg.MaxConnIdleTime,
		"Idle connections are closed after this interval.")
	fs.DurationVar(&o.cfg.HealthCheckPeriod, "postgres-health-check-period", o.cfg.HealthCheckPeriod,
		"How often the pool prunes broken connections.")
}

// Validate runs flag-level checks. Only invoked when --storage-backend=postgres.
func (o *Options) Validate() []error {
	var errs []error
	if o.cfg.DSN == "" {
		errs = append(errs, fmt.Errorf("--postgres-dsn is required when --storage-backend=postgres"))
	}
	return errs
}

// Build returns the per-resource Factory for CR storage. Applies the schema,
// dials the pool, and starts the feed + TTL scanner. Shares the resulting
// FactoryBackend with BuildFactoryBackend so a single runtime backs both CR
// storage and internal factory.Register dispatch.
